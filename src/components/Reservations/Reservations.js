        ...
        <div id='reservations' className="w-full h-[70vh] relative bg-fixed bg-right bg-no-repeat bg-cover bg-[url('./assets/img/reservations.jpg')]">
            <div className='w-4/5 text-white-0 flex flex-col items-center translate-x-[-50%] translate-y-[-50%] absolute left-[50%] top-[50%]'>
                ...
                <div className='w-full font-Vazir-digit flex flex-col my-[50px] sm:w-4/5 sm:flex-row lg:w-1/2'>
                    <input name='date' value={data.date} onChange={changeHandler} className='flex-1 p-[10px] bg-transparent-0 border-2 border-orange-0 outline-none' type='date' />
                    <input name='time' value={data.time} onChange={changeHandler} className='flex-1 p-[10px] bg-transparent-0 border-2 border-orange-0 outline-none my-[10px] sm:my-0 sm:mx-[10px]' type='time' />
                    <select name='select' value={data.select} onChange={changeHandler} className='flex-1 p-[10px] bg-transparent-0 border-2 border-orange-0 outline-none'>
